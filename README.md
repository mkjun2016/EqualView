...
| 분류 | 기술 | 용도 |
|------|------|------|
| **백엔드 프레임워크** | Python 3.12, FastAPI | REST API 서버 |
| **비동기 작업** | Celery + Redis | 영상 처리 비동기 파이프라인 |
| **대사 추출** | faster-whisper (base) | 음성 전사 + 침묵 구간 감지 |
| **얼굴 인식** | InsightFace (buffalo_l) + onnxruntime | 인물 식별 및 추적 |
| **화면해설 생성** | Gemini 2.5 Flash (google-genai) | 멀티모달 한국어 해설 생성 |
| **TTS** | edge-tts (`ko-KR-SunHiNeural`) | 무료 한국어 음성 합성 |
| **영상/오디오 처리** | ffmpeg (imageio-ffmpeg) | 오디오 추출 · 영상 합성 |
| **프론트엔드** | React 18, Vite 5 | 업로드 · 진행 표시 · 결과 재생 |
